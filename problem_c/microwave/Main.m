...
for i = real_products_in_total:real_products_in_total+500
    new_customer_polarity = normrnd(microwave_polarity_fit.mu,microwave_polarity_fit.sigma);
    new_customer_subjectivity = normrnd(microwave_subjectivity_fit.mu,microwave_subjectivity_fit.sigma);
    % I used the curve fitting tool to get the relationship between
    % ploarity and rating
    new_customer_star_rating = floor(0.5949*new_customer_polarity + 2.969);
...
future_ratings = virtual_product.reviews(real_products_in_total:real_products_in_total+500,1);
